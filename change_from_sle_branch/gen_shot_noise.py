...
def amp_ta(
    ...
    distlist = ['exp', 'deg', 'ray', 'unif', 'unifc', 'gam', 'pareto', 'alap']
    ...
    if TWdist == 'exp':
        ...
        # TW = sample_bounded_Pareto(alpha=TWkappa, L=1e-3, H=tw, size=K, seed=seedTW)
        TW = sample_bounded_Pareto(alpha=TWkappa, L=1e-3, H=5e4, size=K, seed=seedTW)
        TW *= tw / TW.mean()
        print(TW.min(), TW.max())
        # Save largest and smallest waiting times
        # np.savez('/home/een023/Documents/work/FPP_SOC_Chaos/report/data/final_g001_minmax.npz',
        #          tw_min=TW.min(), tw_max=TW.max())
...
def kern(tkern, kerntype=0, lam=0.5, dkern=False, tol=1e-5):
    ...
    # Experimental
        ...
        tkern_ = np.r_[tkern[1:], tkern[-1] + tkern[-1] - tkern[-2]]
        assert len(tkern_) == len(tkern)
        kern[tkern >= 0] = tkern_[tkern >= 0]**(- 1)
