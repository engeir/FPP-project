...
def save_fig(fig, file_name, fmt=None, dpi=300, tight=True):
    ...
    if fmt == "eps":
        subprocess.call(
            "epstool --bbox --copy {} {}".format(tmp_name, file_name), shell=True
        )
    elif fmt == "png":
        subprocess.call("convert {} -trim {}".format(tmp_name, file_name), shell=True)
    elif fmt == "pdf":
        subprocess.call("pdfcrop {} {}".format(tmp_name, file_name), shell=True)
